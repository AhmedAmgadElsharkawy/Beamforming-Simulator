...
class ArrayStrategy:
    @staticmethod
    def calculate_linear_weights(wave_number: float, params: BeamformingParameters, steering_angle: float) -> np.ndarray:
        phase = wave_number * params.spacing * np.arange(params.elements) * np.sin(steering_angle)
        return np.exp(-1j * phase)

    @staticmethod
    def calculate_curved_weights(wave_number: float, params: BeamformingParameters, steering_angle: float) -> np.ndarray:
...
class BeamformingModel:
    def __init__(self, array_model: ArrayModel):
    ...
    def _get_wavenumber(self, frequency: float) -> float:
    ...
    def _get_curved_params(self, elements: int, curvature: float) -> Tuple[float, float]:
    ...
    def calculate_steering_vector(self, params: BeamformingParameters, use_phase: bool = False) -> np.ndarray:
    ...
    def _calculate_linear_steering_vector(self, wave_number: float, params: BeamformingParameters, phase: float) -> np.ndarray:
    ...
    def _calculate_curved_steering_vector(self, wave_number: float, params: BeamformingParameters, phase: float) -> np.ndarray:
    ...
    def calculate_weights(self, params: BeamformingParameters, steering_angle: float) -> np.ndarray:
    ...
    def calculate_pattern(self, params: BeamformingParameters, angles: np.ndarray, use_phase: bool) -> np.ndarray:
    ...
    def calculate_interference_pattern(self, params: BeamformingParameters) -> np.ndarray:
        grid = self._setup_interference_grid()
        return self._calculate_interference_factors(grid, params)
    
    def _setup_interference_grid(self) -> Dict[str, np.ndarray]:
    ...
    def _calculate_interference_factors(self, grid: Dict[str, np.ndarray], params: BeamformingParameters) -> np.ndarray:
    ...
    def _calculate_vectorized_array_factor(self, params: BeamformingParameters, steering_angle: np.ndarray, wave_number: float) -> np.ndarray:
    ...
    def _calculate_array_factor(self, wave_number: float, spacing: float, n_array: np.ndarray,
        steering_angle: np.ndarray, phase_diff: float,
        element_phase: float) -> np.ndarray:
        return np.sum(np.exp(1j * (wave_number * spacing * n_array * np.sin(steering_angle) +
             n_array * phase_diff + element_phase)), axis=0)

    def _init_cache(self, cache_name: str) -> dict:
        if not hasattr(self, cache_name):
            setattr(self, cache_name, {})
        return getattr(self, cache_name)
    
    def _calculate_phase(self, phase: float, use_phase: bool = True) -> float:
        return np.deg2rad(phase) if use_phase else 0
    
    def _calculate_phase_difference(self, wave_number: float, params: BeamformingParameters) -> float:
